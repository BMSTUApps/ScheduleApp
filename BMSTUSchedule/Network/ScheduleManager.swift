...
class ScheduleManager {
    ...
    func addLesson(lesson: Lesson, group: Group, weekKind: Week.Kind, dayTitle: Day.Title) {
        ...
        let lessonRef = FIRDatabase.database().reference(withPath: "schedules").child(group.name).child(weekKind.rawValue).child(dayTitle.rawValue).child(lesson.title)
    ...
    func randomSchedule() -> Schedule {
        ...
        for i in 0...daysTitles.count-1 {
            ...
            for _ in 0...dayLessonsCount {
                ...
                // Add to firebase
                ScheduleManager.sharedManager.addLesson(lesson: lesson!, group: Group(name: "ИУ5-33"), weekKind: .denominator, dayTitle: daysTitles[i])
